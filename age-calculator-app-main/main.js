...
function convertArrayElements(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
      if (typeof arr[i] === 'string') {
          // If the element is a string, convert it to a number
          ...
          // If the element is a number, convert it to a string
          arr[i] = arr[i].toString();
      }
      // If the element is neither a string nor a number, leave it as is
  }
  return arr;
}
